...
inp_mask_dir = './sample_data/sample_IF_groundtruth_masks/'
out_dir = './sample_data/outputs/NTM_csvs'
...
out_file_path = os.path.join(out_dir, 'all_Myofibres_Calculations_QIF' + '.csv')
...
df_ = pd.read_csv(out_file_path, index_col=None, header=0)
df_['NTM']= ((df_.Fiber_Area>210)&(df_.Fiber_Area<27000)&(df_.Convexity<1.19)& (df_.lengthsquared_by_area_Bbox<3.10)& (df_raw.Fiber_on_Border == False) & (~((df_.Aspect_Ratio_Ellipse>2.2)& (df_.lengthsquared_by_area_Bbox>1.8))))

df_.to_csv('./sample_data/outputs/NTM_csvs/NTM_detection.csv',sep=',')
